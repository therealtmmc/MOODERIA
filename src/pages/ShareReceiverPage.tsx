...
export default function ShareReceiverPage() {
 ...
 <div className="relative bg-white w-32 h-32 rounded-full flex items-center justify-center shadow-2xl border-purple-300 group-hover:scale-105 transition-transform">
 ...
 <h2 className="text-white font-black text-2xl mt-6 uppercase tracking-widest drop-">
 ...
 className="w-48 h-32 bg-white rounded-lg shadow-2xl -t-[40px] -t-gray-200 relative"
 >
 ...
 className="w-48 h-64 bg-white rounded-xl shadow-2xl origin-top -t-8 -t-gray-800"
 />
 )}
 {payload.type ==='event' && (
 <motion.div
 ...
 className="w-48 h-48 bg-white rounded-3xl shadow-2xl -t-[30px] -t-red-500 flex items-center justify-center"
 >
 ...
 className="w-48 h-64 bg-white rounded-xl shadow-2xl -t-8 -t-amber-500 flex flex-col items-center justify-start pt-4"
 >
 ...
 transition={{ duration: 1.5, ease:"easeInOut" }}
 className="w-48 h-48 bg-white rounded-full shadow-2xl border-indigo-500 flex items-center justify-center"
 >
...
"p-6 text-white flex justify-between items-center",
 ...
 <div className="flex items-center justify-between -b-2 pb-4">
 ...
 <img src={payload.data.image} alt="Diary" className="w-full rounded-2xl mt-4" />
 ) : (
 <div className="bg-gray-100 p-4 rounded-2xl -dashed text-center text-xs font-bold text-gray-400 mt-4">
 {payload.data.image}
 </div>
 )
 )}
 ...
 <video src={payload.data.video} controls playsInline className="w-full rounded-2xl mt-4 bg-black" />
 ) : (
 <div className="bg-gray-100 p-4 rounded-2xl -dashed text-center text-xs font-bold text-gray-400 mt-4">
 {payload.data.video}
 </div>
...
'bg-red-100 text-red-700'
 ...
 <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center font-black text-gray-400">
 {idx + 1}
 </div>
 ...
 <div className="w-24 h-24 bg-blue-50 rounded-3xl flex flex-col items-center justify-center mx-auto border-blue-100 shadow-inner">
 ...
 <div className="bg-indigo-50 p-4 rounded-2xl border-indigo-100 flex items-center gap-4">
 ...
 <div className="bg-red-50 border-red-200 p-3 rounded-xl flex items-start gap-2 text-red-700 text-xs font-bold text-left mt-4">
 ...
 className="p-6 bg-gray-50 -t-2"
 >
 <button
 onClick={handleSave}
 disabled={payload.type ==='market' && state.walletBalance < payload.data.items.reduce((acc: number, item: any) => acc + (item.price || 0), 0)}
 className={cn(
"w-full py-4 rounded-2xl font-black text-white uppercase tracking-widest flex items-center justify-center gap-2 active:scale-95 transition-transform disabled:opacity-50 disabled:active:scale-100",
 payload.type ==='diary' ?'bg-purple-600 hover:bg-purple-700' :
 payload.type ==='workout' ?'bg-red-500 hover:bg-red-600' :
 payload.type ==='market' ?'bg-amber-500 hover:bg-amber-600' :
 payload.type ==='passport' ?'bg-indigo-600 hover:bg-indigo-700' :
'bg-blue-500 hover:bg-blue-600'
 ...
 <div className="w-20 h-20 bg-white rounded-3xl flex items-center justify-center mx-auto mb-6 border-amber-100">
 ...
 You must create your <b>MOODERIA</b> profile first to save this {payload.type} to your city!
 </p>

 <button
 onClick={() => navigate('/onboarding')}
 className="w-full py-4 bg-amber-500 text-white rounded-2xl font-black uppercase tracking-widest hover:bg-amber-600 transition-colors flex items-center justify-center gap-2 active:scale-95"
 >
