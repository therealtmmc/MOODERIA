...
const RANKS = [
 ...
 { id: 2, name:"Consistent Doer", icon:"🪵", color:"text-amber-700", bg:"bg-amber-100", border: "", threshold: 20 },
...
export default function ProfilePage() {
 ...
 const isGold = state.profileBorder ==='_gold';
 assets.push({
 id: state.profileBorder,
 nameborder: isGold ?'Gold Border' :'Diamond Border',
 icon: Crown,
 colorborder: isGold ?'text-yellow-600' :'text-cyan-500',
 bgborder: isGold ?'bg-yellow-50' :'bg-cyan-50',
 ...
 const hospital = state.workouts.filter(w => isSameDay(new Date(w.date), day)).reduce((acc, w) => acc + (w.duration || 0), 0);
 ...
 {state.isStarkTheme ?"SYSTEM.RECORDS.READ()" : <>Official <b>MOODERIA</b> Records</>}
 </p>
 </header>
...
"relative p-6 space-y-6 overflow-hidden",
 state.isStarkTheme 
 ?"bg-black/90 backdrop-blur-md rounded-none border-green-500/30 shadow-[0_0_20px_rgba(0,255,65,0.1)] font-mono"
 : cn("bg-white rounded-[2.5rem]", currentRankData.border ||"")
 )}>
 {/* Background Gradient */}
 {!state.isStarkTheme && (
 ...
 {/* User Photo */}
 <div className={cn(
"w-24 h-24 rounded-3xl overflow-hidden shrink-0 relative",
 state.isStarkTheme ?"bg-black border-green-500/50 rounded-none shadow-[0_0_15px_rgba(0,255,65,0.2)]" :"bg-gray-200",
 !state.isStarkTheme && state.profileBorder ==="_gold" ?" shadow-[0_0_15px_rgba(212,175,55,0.5)]" :
 !state.isStarkTheme && state.profileBorder ==="_diamond" ?" shadow-[0_0_15px_rgba(185,242,255,0.8)]" :
 ...
 <div className="absolute -bottom-2 -right-2 bg-transparent rounded-full p-1">
 <div className={cn("text-[10px] font-black px-2 py-0.5 rounded-full flex items-center gap-1", state.isStarkTheme ?"bg-black border-green-500 text-green-400 rounded-none" :"bg-orange-500 text-white")}>
 ...
 <span className={cn("px-2 py-1 text-[10px] font-black uppercase", state.isStarkTheme ?"bg-green-900/20 text-green-400 border-green-500/30 rounded-none" :"bg-blue-100 text-blue-600 rounded-lg border-blue-200")}>
 Lvl {Math.floor(totalTasks / 10) + 1}
 </span>
 <span className={cn("px-2 py-1 text-[10px] font-black uppercase", state.isStarkTheme ?"bg-green-900/20 text-green-400 border-green-500/30 rounded-none" :"bg-purple-100 text-purple-600 rounded-lg border-purple-200")}>
 ...
 className={cn("w-20 h-20 flex items-center justify-center text-4xl relative z-10", state.isStarkTheme ?"bg-black border-green-500/50 rounded-none" : cn("rounded-full border-white bg-white", currentRankData.bg))}
 >
 {state.isStarkTheme ? <Terminal className="w-8 h-8 text-green-500" /> : currentRankData.icon}
 <div className={cn("absolute -bottom-2 text-[10px] font-black px-2 py-0.5 uppercase tracking-wider", state.isStarkTheme ?"bg-black text-green-400 border-green-500 rounded-none" :"bg-black text-white rounded-full border-white")}>
 Rank {currentRank + 1}
 </div>
 </motion.div>
 </div>
 ...
 <div className={cn("h-6 w-full overflow-hidden relative", state.isStarkTheme ?"bg-black border-green-500/30 rounded-none" :"bg-gray-100 rounded-full shadow-inner")}>
 <motion.div 
 initial={{ width: 0 }}
 ...
 <span className={cn("text-[9px] font-black drop-", state.isStarkTheme ?"text-black" :"text-white")}>{Math.round(progress)}%</span>
 </motion.div>
 </div>
 </div>
 </div>
...
"relative overflow-hidden p-6",
 ...
 <Tooltip 
 contentStyle={{ 
 Radius: state.isStarkTheme ?'0px' :'12px', 
 ...
 <div className={cn("p-3 flex items-center gap-3", state.isStarkTheme ?"bg-green-900/20 border-green-500/30 rounded-none" :"bg-red-50 rounded-2xl border-red-100")}>
 <div className={cn("p-2", state.isStarkTheme ?"bg-black border-green-500/50 text-green-500 rounded-none" :"bg-red-100 rounded-xl text-red-600")}><Sword className="w-4 h-4" /></div>
 <div>
 ...
 <div className={cn("p-3 flex items-center gap-3", state.isStarkTheme ?"bg-green-900/20 border-green-500/30 rounded-none" :"bg-blue-50 rounded-2xl border-blue-100")}>
 <div className={cn("p-2", state.isStarkTheme ?"bg-black border-green-500/50 text-green-500 rounded-none" :"bg-blue-100 rounded-xl text-blue-600")}><Brain className="w-4 h-4" /></div>
 <div>
 ...
 <div className={cn("p-3 flex items-center gap-3", state.isStarkTheme ?"bg-green-900/20 border-green-500/30 rounded-none" :"bg-green-50 rounded-2xl border-green-100")}>
 <div className={cn("p-2", state.isStarkTheme ?"bg-black border-green-500/50 text-green-500 rounded-none" :"bg-green-100 rounded-xl text-green-600")}><Shield className="w-4 h-4" /></div>
 <div>
 ...
 <div className={cn("p-3 flex items-center gap-3", state.isStarkTheme ?"bg-green-900/20 border-green-500/30 rounded-none" :"bg-orange-50 rounded-2xl border-orange-100")}>
 <div className={cn("p-2", state.isStarkTheme ?"bg-black border-green-500/50 text-green-500 rounded-none" :"bg-orange-100 rounded-xl text-orange-600")}><Zap className="w-4 h-4" /></div>
 <div>
 ...
 <div className={cn("p-4 flex items-center justify-between col-span-2", state.isStarkTheme ?"bg-green-900/20 border-green-500/30 rounded-none" :"bg-emerald-50 rounded-2xl border-emerald-100")}>
 <div className="flex items-center gap-3">
 <div className={cn("p-2", state.isStarkTheme ?"bg-black border-green-500/50 text-green-500 rounded-none" :"bg-emerald-100 rounded-xl text-emerald-600")}><DollarSign className="w-5 h-5" /></div>
 <div>
...
"relative overflow-hidden p-6",
 ...
 className={cn(
"min-w-[240px] p-5 rounded-2xl snap-center shrink-0 flex flex-col gap-3",
 state.isStarkTheme ?"bg-black border-green-500/30" :"bg-white"
 )}
 >
 <h4 className={cn(
"font-bold text-lg -b pb-2",
 state.isStarkTheme ?"text-green-400 border-green-500/30 font-mono" :"text-gray-800"
 )}>
 {format(dayData.date,'EEEE, MMM d')}
 </h4>
 <div className="grid grid-cols-2 gap-2 text-sm mt-2">
 <div className={cn("p-2 rounded-xl flex flex-col items-center justify-center text-center", state.isStarkTheme ?"bg-green-900/20 border-green-500/30" :"bg-blue-50")}>
 <span className={cn("text-[10px] uppercase font-bold", state.isStarkTheme ?"text-green-600/70" :"text-blue-400")}>Works done</span>
 <span className={cn("font-black text-lg", state.isStarkTheme ?"text-green-400" :"text-blue-700")}>{dayData.works}</span>
 </div>
 <div className={cn("p-2 rounded-xl flex flex-col items-center justify-center text-center", state.isStarkTheme ?"bg-green-900/20 border-green-500/30" :"bg-red-50")}>
 <span className={cn("text-[10px] uppercase font-bold", state.isStarkTheme ?"text-green-600/70" :"text-red-400")}>General Hospital</span>
 <span className={cn("font-black text-lg", state.isStarkTheme ?"text-green-400" :"text-red-700")}>
   {dayData.hospital >= 3600 ? `${Math.floor(dayData.hospital / 3600)}h ` : ''}{Math.floor((dayData.hospital % 3600) / 60)}m
 </span>
 </div>
 <div className={cn("p-2 rounded-xl flex flex-col items-center justify-center text-center", state.isStarkTheme ?"bg-green-900/20 border-green-500/30" :"bg-purple-50")}>
 <span className={cn("text-[10px] uppercase font-bold", state.isStarkTheme ?"text-green-600/70" :"text-purple-400")}>How long reviewing</span>
 <span className={cn("font-black text-lg", state.isStarkTheme ?"text-green-400" :"text-purple-700")}>{dayData.reviewing}</span>
 </div>
 <div className={cn("p-2 rounded-xl flex flex-col items-center justify-center text-center", state.isStarkTheme ?"bg-green-900/20 border-green-500/30" :"bg-amber-50")}>
 <span className={cn("text-[10px] uppercase font-bold", state.isStarkTheme ?"text-green-600/70" :"text-amber-400")}>Market district</span>
 <span className={cn("font-black text-lg", state.isStarkTheme ?"text-green-400" :"text-amber-700")}>${dayData.market.toFixed(0)}</span>
 </div>
 <div className={cn("p-2 rounded-xl flex flex-col items-center justify-center text-center", state.isStarkTheme ?"bg-green-900/20 border-green-500/30" :"bg-emerald-50")}>
 <span className={cn("text-[10px] uppercase font-bold", state.isStarkTheme ?"text-green-600/70" :"text-emerald-400")}>How much savings</span>
 <span className={cn("font-black text-lg", state.isStarkTheme ?"text-green-400" :"text-emerald-700")}>${dayData.savings.toFixed(0)}</span>
 </div>
 <div className={cn("p-2 rounded-xl flex flex-col items-center justify-center text-center", state.isStarkTheme ?"bg-green-900/20 border-green-500/30" :"bg-rose-50")}>
 <span className={cn("text-[10px] uppercase font-bold", state.isStarkTheme ?"text-green-600/70" :"text-rose-400")}>Expenses</span>
 <span className={cn("font-black text-lg", state.isStarkTheme ?"text-green-400" :"text-rose-700")}>${dayData.expenses.toFixed(0)}</span>
 </div>
 </div>
 </div>
 ))}
...
"relative overflow-hidden p-6",
 ...
 <div key={badge.id} className={cn("p-3 flex flex-col items-center justify-center text-center gap-2", state.isStarkTheme ?"bg-green-900/20 border-green-500/30" :"bg-yellow-50 rounded-2xl border-yellow-100")}>
 ...
 className={cn(
"w-full py-4 font-black uppercase tracking-widest active:scale-95 transition-transform flex items-center justify-center gap-2",
 state.isStarkTheme 
 ?"bg-black text-green-500 border-green-500 rounded-none hover:bg-green-900/20 shadow-[0_0_15px_rgba(0,255,65,0.2)]" 
 :"bg-indigo-600 text-white rounded-2xl hover:bg-indigo-700"
 )}
 >
