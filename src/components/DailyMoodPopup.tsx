...
const MOODS = [
 ...
 { label:"Tired", icon: Coffee, color:"bg-white", contentColor:"text-gray-500" },
...
export function DailyMoodPopup() {
 ...
 className={cn(
"flex flex-col items-center justify-center p-4 rounded-2xl transition-transform active:scale-95 border-black/10",
 mood.color
 )}
 >
 <mood.icon className={cn("w-8 h-8 drop- mb-2", mood.contentColor)} />
 <span className={cn("font-black text-sm uppercase tracking-wide shadow-black drop-", mood.contentColor)}>
 {mood.label}
 </span>
 </button>
 ))}
