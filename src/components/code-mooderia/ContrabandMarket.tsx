...
export function ContrabandMarket({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="bg-black/95 backdrop-blur-xl rounded-none sm:rounded-xl border border-red-500/50 p-4 sm:p-6 font-mono text-red-500 h-[75vh] min-h-[400px] max-h-[600px] flex flex-col shadow-[0_0_30px_rgba(255,0,0,0.15)] relative overflow-hidden">
      {/* Scanline effect */}
      ...
      <div className="flex justify-between items-center border-b border-red-500/30 pb-3 sm:pb-4 mb-4 sm:mb-6 relative z-20">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-3 sm:space-y-4 relative z-20">
        {MARKET_ITEMS.map((item) => {
          ...
          return (
            <div key={item.id} className="bg-red-950/20 border border-red-500/20 p-3 sm:p-4 rounded-lg flex items-center justify-between hover:border-red-500/50 transition-colors group gap-2">
              ...
              <div className="flex flex-col items-end gap-1 sm:gap-2 shrink-0">
                ...
                <button
                  ...
                  disabled={isDisabled}
                  className={`px-2 py-1 sm:px-4 sm:py-1.5 text-[10px] sm:text-xs font-bold uppercase tracking-widest rounded border transition-all ${
                    isOwned
