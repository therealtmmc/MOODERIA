...
export function Header({ onMenuClick }: { onMenuClick: () => void }) {
  ...
  return (
    <div className="px-4 pt-4 pb-2 sticky top-0 z-30 transition-all duration-[3000ms]">
      <div className={cn(
        "backdrop-blur-md p-4 shadow-lg transition-all duration-[3000ms]",
        state.isStarkTheme 
          ? "bg-black/90 rounded-none border border-green-500/30 shadow-[0_0_15px_rgba(34,197,94,0.1)]" 
          : "bg-white/90 rounded-[2.5rem] border-4 border-gray-100"
      )}>
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2">
            <button 
              ...
              className={cn(
                ...
                state.isStarkTheme
                  ? "text-green-400 bg-transparent border-none p-0"
                  : "w-10 h-10 bg-[#46178f] text-white rounded-xl transform -rotate-3 border-2 border-[#2f0f60] shadow-md"
              )}
            ...
            <div className="flex items-center">
              <h1 className={cn(
                "font-black text-xl uppercase leading-none transition-colors duration-[3000ms]",
                state.isStarkTheme ? "text-green-400 font-mono tracking-widest stark-theme-glitch ml-1" : "tracking-tighter text-[#46178f]"
              )}>
          ...
          <button 
            ...
            className={cn(
              ...
              state.isStarkTheme
                ? "bg-black text-green-400 border border-green-500/50 hover:bg-green-900/30 rounded-none"
                : "bg-gray-100 rounded-xl text-gray-700 hover:bg-[#46178f] hover:text-white border-2 border-gray-200"
            )}
          >
        ...
        <div className={cn(
          ...
          state.isStarkTheme
            ? "bg-black border border-green-500/30 rounded-none"
            : "bg-gray-900 rounded-2xl shadow-inner border-2 border-gray-800"
        )}>
            {!state.isStarkTheme && <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-20"></div>}
