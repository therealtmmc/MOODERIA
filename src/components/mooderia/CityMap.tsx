...
export function CityMap() {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-md rounded-3xl p-6 shadow-xl border border-white/50 space-y-6">
      ...
      <div className="relative w-full aspect-square bg-slate-50 rounded-2xl overflow-hidden border border-slate-200">
        ...
        {DISTRICTS.map((district) => {
          ...
          return (
            <motion.button
              ...
              whileHover={{ y: -5 }}
            >
              <div className={`p-2 rounded-full shadow-lg ${isActive ? district.bg : 'bg-white'}`}>
                <Icon className={`w-5 h-5 ${isActive ? district.color : 'text-slate-400'}`} />
              </div>
              {isActive && (
                <div className="bg-white px-2 py-1 rounded-md shadow-md text-[10px] font-bold text-slate-800 whitespace-nowrap">
                  {district.name}
                </div>
              )}
            </motion.button>
      ...
      <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
        ...
        <div className="flex justify-between items-end">
          ...
          <button className="text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 px-4 py-2 rounded-xl transition-colors shadow-sm">
            Fast Travel
          </button>
        </div>
      </div>
